...
import type { StudentData, Assessment, AttendanceRecord, Announcement, MonthlyExam } from '../App';
import { SUBJECTS, GRADES } from '../App'; // Import Constants
...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ 
    students, teacherSubject, onUpdateStudent, onBulkUpdate, onDeleteStudent, onLogout, triggerToast, undo, redo, canUndo, canRedo 
}) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
        if ((e.ctrlKey || e.metaKey) && e.key === 'z') { e.preventDefault(); if (canUndo) { undo(); triggerToast('تم التراجع'); } }
        if ((e.ctrlKey || e.metaKey) && e.key === 'y') { e.preventDefault(); if (canRedo) { redo(); triggerToast('تم إعادة الأمر'); } }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [canUndo, canRedo, undo, redo, triggerToast]);
        ...
        <div className="p-6 pb-2 space-y-4">
          <div className="flex justify-between items-center">
             ...
             <div className="flex gap-1 bg-slate-50 p-1 rounded-xl border border-slate-100">
                 <button onClick={undo} disabled={!canUndo} className="p-2 hover:bg-white hover:shadow-sm rounded-lg text-slate-400 hover:text-slate-800 disabled:opacity-30 transition-all"><RotateCcw className="w-3.5 h-3.5" /></button>
                 <button onClick={redo} disabled={!canRedo} className="p-2 hover:bg-white hover:shadow-sm rounded-lg text-slate-400 hover:text-slate-800 disabled:opacity-30 transition-all"><RotateCw className="w-3.5 h-3.5" /></button>
             </div>
          </div>
