...
export function StaffLogin({ onSuccess, onBack, onRegister }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const { token, user } = await staffLogin(username.trim(), password);
      setStaffToken(token);
      onSuccess(user);
    } catch (err) {
    ...
    <div className="mx-auto max-w-md animate-rise">
      ...
      <div className="card-accent relative overflow-hidden rounded-[2rem] glass-strong p-8 md:p-10">
        <div className="mb-8 text-center">
          ...
          <h2 className="text-[clamp(1.75rem,4vw,2.25rem)] font-black tracking-tight text-white">دخول الإدارة</h2>
          <p className="mt-2 text-sm text-slate-400">سجّل دخولك بحساب الإدارة للمتابعة</p>
        </div>
